...
public class StackActivity extends AppCompatActivity {
	...
	private boolean pairStackRuleCheck(View v, View check) { //Checks if the pair of Views break a rule. false if they don't, true if they do.
		for (Rule rule : rules) {
			if (rule.equals(new Rule((int) (v.getTag(R.id.stack_value)), (int) (check.getTag(R.id.stack_value))))) {
				return true;
			}
		}
		return false;
	...
	private void newGameTile() {
		...
		gameTile.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), color));
	...
	private Rule newRule(int i) {
		boolean works = false;
		Rule a = new Rule((int) (Math.random() * 9) + 1, (int) (Math.random() * 9) + 1);
		while (!works) {
			boolean unique = true;
			for (Rule compare : rules) {
				if (a.equals(compare)) {
					unique = false;
					a = new Rule((int) (Math.random() * 9) + 1, (int) (Math.random() * 9) + 1);
				}
			}
