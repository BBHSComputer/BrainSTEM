	...
	public void startGame(final int song) { //Creates a mediaplayer of the song, playing back all of its notes through an array and a time delay handler
		if (song == 1) {
            ...
            for (int i : arr) {
				...
				}, i); // Create a note at the specified time (from the array)
			}
        } else if (song == 2) {
